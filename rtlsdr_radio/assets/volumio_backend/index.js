...
function CoreCommandRouter (server) {
  ...
  this.dynamicMenuItems = []; // RTL-SDR Radio: Support for plugin-added menu items
...
CoreCommandRouter.prototype.getMenuItems = function () {
  ...
  self.i18nJson(__dirname + '/i18n/strings_' + lang_code + '.json',
    ...
    .then(function (menuItemsJson) {
      ...
      // RTL-SDR Radio: Merge dynamic menu items
...
// RTL-SDR Radio: Dynamic menu item support
CoreCommandRouter.prototype.pushMenuItems = function (items) {
  var self = this;
  ...
  items.forEach(function(item) {
    // Remove existing item with same ID and name
  ...
  self.logger.info('[CommandRouter] Dynamic menu items updated: ' + self.dynamicMenuItems.length + ' items');
};

CoreCommandRouter.prototype.removeMenuItems = function (id, name) {
  var self = this;
  ...
  self.logger.info('[CommandRouter] Dynamic menu item removed: ' + id + ' / ' + name);
};
